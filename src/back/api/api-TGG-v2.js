...
export const BASE_URL_API_TGG_V2 = "/api/v2/literacy-rates";
...
export function loadBackendTGGv2(app) {
    ...
    // GET /api/v2/literacy-rates/docs - Redirección a la documentación (si existe)
    app.get(BASE_URL_API_TGG_V2 + "/docs", (req, res) => {
        res.redirect("https://documenter.getpostman.com/view/52263638/2sBXigMDfw"); // Cambiar a la URL de v2 cuando esté disponible
    });

    // GET /api/v2/literacy-rates/loadInitialData
    app.get(BASE_URL_API_TGG_V2 + "/loadInitialData", (req, res) => {
        db.find({}, (err, docs) => {
            ...
            if (docs.length > 0) {
                ...
                const initialData = [
                    { country: "Armenia", total: 99.8, male: 99.8, female: 99.7, gender_gap: 0.1, year: 2021 },
                    { country: "Colombia", total: 95.6, male: 95.4, female: 95.9, gender_gap: 0.5, year: 2020 },
                    { country: "Ecuador", total: 93.6, male: 94.8, female: 92.5, gender_gap: 2.3, year: 2012 },
                    { country: "Indonesia", total: 96.0, male: 97.4, female: 94.6, gender_gap: 2.8, year: 2015 },
                    { country: "Kuwait", total: 96.5, male: 97.1, female: 95.4, gender_gap: 1.7, year: 2020 },
                    { country: "Mexico", total: 95.2, male: 96.1, female: 94.5, gender_gap: 1.6, year: 2013 },
                    { country: "Mongolia", total: 99.2, male: 99.1, female: 99.2, gender_gap: 0.1, year: 2014 },
                    { country: "North Macedonia", total: 98.4, male: 99.1, female: 97.6, gender_gap: 1.5, year: 2018 },
                    { country: "Paraguay", total: 94.5, male: 94.9, female: 94.2, gender_gap: 0.7, year: 2018 },
                    { country: "Peru", total: 94.5, male: 97.0, female: 92.0, gender_gap: 5.0, year: 2014 },
                    { country: "Saudi Arabia", total: 97.6, male: 98.6, female: 96.0, gender_gap: 2.6, year: 2016 },
    ...
    // GET /api/v2/literacy-rates (con filtros y paginación)
    app.get(BASE_URL_API_TGG_V2, (req, res) => {
        const query = {};
        let offset = 0;
        let limit = 0;
    ...
    // GET /api/v2/literacy-rates/:country (filtros year/from/to y paginación)
    app.get(BASE_URL_API_TGG_V2 + "/:country", (req, res) => {
    ...
    // GET /api/v2/literacy-rates/:country/:year
    app.get(BASE_URL_API_TGG_V2 + "/:country/:year", (req, res) => {
        const countryParam = req.params.country;
    ...
    // POST /api/v2/literacy-rates
    app.post(BASE_URL_API_TGG_V2, (req, res) => {
    ...
    // PUT /api/v2/literacy-rates (No permitido)
    app.put(BASE_URL_API_TGG_V2, (req, res) => {
        res.status(405).json({ error: "Method Not Allowed: No se puede actualizar la lista completa" });
    });

    // DELETE /api/v2/literacy-rates
    app.delete(BASE_URL_API_TGG_V2, (req, res) => {
    ...
    // POST /api/v2/literacy-rates/:country/:year (No permitido)
    app.post(BASE_URL_API_TGG_V2 + "/:country/:year", (req, res) => {
        res.status(405).json({ error: "Method Not Allowed: No se puede hacer POST a un recurso concreto." });
    });

    // PUT /api/v2/literacy-rates/:country/:year
    app.put(BASE_URL_API_TGG_V2 + "/:country/:year", (req, res) => {
        const countryParam = req.params.country;
        const yearParam = parseInt(req.params.year);
        const body = req.body;
    ...
    // DELETE /api/v2/literacy-rates/:country/:year
    app.delete(BASE_URL_API_TGG_V2 + "/:country/:year", (req, res) => {
        const countryParam = req.params.country;
        const yearParam = parseInt(req.params.year);
